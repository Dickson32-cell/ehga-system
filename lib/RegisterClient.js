            ...
            <div className="form-grid">
              {def.fields.map((f) => {
                ...
                if (f.type === "select") {
                  ...
                  // Field-staff dropdown (active drivers & riders) — used to link a
                  // vehicle to the person HR added. Names come from /api/staff-directory.
                  input = (
                    <select {...common}>
                      <option value="">- none -</option>
                      {fieldStaff.map((p) => (
                        <option key={p.full_name} value={p.full_name}>
                          {p.full_name} ({p.role === "RIDER" ? "Dispatch Rider" : "Driver"})
                        </option>
                      ))}
                    </select>
                  );
