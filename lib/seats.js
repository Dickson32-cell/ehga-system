...
/** Vehicles that can take passenger seat bookings, in code order. */
export async function bookableVehicles(client) {
  // Only proper passenger cars (intercity/local/private-hire sedans) — bikes and
  // parcel bikes are excluded. MD/OM set primary_role in Fleet.
  const sql = `SELECT vehicle_code, model, color, registration, seat_capacity
       FROM vehicle
      WHERE deleted = FALSE
        AND status = 'Available'
        AND (primary_role ILIKE '%intercity%'
          OR primary_role ILIKE '%local%'
          OR primary_role ILIKE '%school%'
          OR primary_role ILIKE '%private hire%'
          OR primary_role ILIKE '%passenger%'
          OR primary_role ILIKE '%sedan%')
...
export async function seatsTakenByVehicle(client, travelDate, direction) {
  const sql = `SELECT vehicle_id, COALESCE(SUM(seats),0)::int AS taken
       FROM booking
      WHERE deleted = FALSE
        AND travel_date = $1
        AND direction = $2
        ...
        AND status = ANY($3)
      GROUP BY vehicle_id`;
  const params = [travelDate, direction, ACTIVE_BOOKING_STATUSES];
  const { rows } = client
